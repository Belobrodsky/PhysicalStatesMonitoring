...
namespace Ipt
{
    ...
    public static class MbCliWrapper
    {
        ...
        /// <returns>
        ///     Возвращает результат соединения. 0, если соединение удалось и код ошибки, если соединенеие не удалось.
        ///     <para>Для получения описания ошибки следует обрабатывать событие <see cref="Error" />.</para>
        ///     .
        /// </returns>
        ...
        {
            var result = UnsafeNativeMethods.mb_connect(Marshal.StringToHGlobalAnsi(ip.ToString()), port);
            if (result != 0)
